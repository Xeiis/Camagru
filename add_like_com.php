...
if ($donnees = $sql->fetch()) {
    ...
    $sql3 = $dbh->prepare('SELECT 1 FROM jaime_com WHERE id_utilisateur = ?');
    $sql3->execute(array($id_uti));
    if ($donnees3 = $sql3->fetch()) {
        $jaime -=1;
        $sql4 = $dbh->prepare('DELETE FROM jaime_com WHERE id_utilisateur = ?');
        $sql4->execute(array($id_uti));
    }
    else
    {
        $jaime +=1;
        $sql4 = $dbh->prepare('INSERT INTO jaime_com (id_utilisateur,id_image) VALUES (?, ?)');
